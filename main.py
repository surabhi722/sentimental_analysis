...
model_paths = {
    "LSTM": "artifacts/lstm_model.keras",
    "GRU": "artifacts/gru_model.keras",
    "BiGRU": "artifacts/bigru_model.keras",
}
...
def predict_emotion(text: str, models, tokenizer):
    """Ensemble prediction: average the softmax probabilities from all
    three models (LSTM, GRU, BiGRU) and take the argmax of the average."""
    cleaned = preprocess_text(text)
    sequence = tokenizer.texts_to_sequences([cleaned])
    padded = pad_sequences(sequence, maxlen=max_sequence_length, padding="post")

    per_model_probs = {}
    prob_sum = np.zeros(len(emotion_labels))
    for name, model in models.items():
        probs = model.predict(padded, verbose=0)[0]
        per_model_probs[name] = {label: float(p) for label, p in zip(emotion_labels, probs)}
        prob_sum += probs

    avg_probs = prob_sum / len(models)
    all_probabilities = {label: float(p) for label, p in zip(emotion_labels, avg_probs)}
    predicted_emotion = max(all_probabilities, key=all_probabilities.get)
    confidence = all_probabilities[predicted_emotion]
    return predicted_emotion, confidence, all_probabilities, per_model_probs
...
<div class="ocean-hero">
    ...
    <div class="ocean-eyebrow"><span class="dot"></span>LSTM + GRU + BiGRU Ensemble · Emotion Analysis</div>
...
if analyze_clicked:
    if not user_text.strip():
        ...
        with st.spinner("Reading the current..."):
            try:
                models, tokenizer = load_artifacts()
                predicted_emotion, confidence, all_probabilities, per_model_probs = predict_emotion(
                    user_text, models, tokenizer
                )
            except FileNotFoundError:
                st.error(
                    "Couldn't find the models or tokenizer in `artifacts/`. "
                    "Make sure `lstm_model.keras`, `gru_model.keras`, `bigru_model.keras` "
                    "and `tokenizer.pkl` are all in place."
                )
                st.stop()
        ...
        with st.expander("See what each individual model predicted"):
            for name, probs in per_model_probs.items():
                top_label = max(probs, key=probs.get)
                top_pct = probs[top_label] * 100
                st.markdown(
                    f"**{name}** → {emotion_meta[top_label]['emoji']} "
                    f"{top_label} ({top_pct:.1f}%)"
                )

st.markdown(
    '<div class="ocean-footer">LSTM + GRU + BiGRU ensemble emotion model · built by Surabhi</div>',
    unsafe_allow_html=True,
)
